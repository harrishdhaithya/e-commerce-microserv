...
Spring Cloud Gateway with JWT validation and routing; the SPA now goes through it only. `cart-service` with its own H2 tables. `order-service` creating orders via **synchronous** calls to inventory and payment (no Kafka yet). `inventory-service` and a stub `payment-service`.
